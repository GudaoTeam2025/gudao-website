(() => {
  ...
  function safeExternalUrl(value, fallback = IG_PROFILE_URL) {
    try {
  ...
  function createCard(item, type, itemIndex = 0) {
    ...
    const categoryElement = isProduct && item.category
    ...
    const actionElement = isProduct
      ...
      : `
        <a
          href="${escapeHtml(safeExternalUrl(item.link))}"
          target="_blank"
  ...
  async function contactGudao(product, button) {
    ...
    try {
      ...
      window.setTimeout(openInstagramMessage, 700);
    } finally {
      window.setTimeout(() => {
  ...
  function showCopyNotice(productName) {
    ...
    notice.innerHTML = `
      ...
      <section class="copy-notice-card">
        ...
        <p class="copy-notice-product">${escapeHtml(productName || "目前植株")}</p>
        <p class="copy-notice-description">
          植株資訊已複製。前往孤島 Instagram 聊天室後，請長按訊息輸入框並選擇「貼上」。
        </p>
        <button class="copy-notice-confirm" type="button">前往 IG 私訊 ↗</button>
      </section>
    `;
